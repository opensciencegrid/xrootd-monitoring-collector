...
class UdpCollector(object):
    ...
    def _bus_child(self, config, message_q: multiprocessing.Queue, metrics_q: multiprocessing.Queue):
        ...
        def on_message(channel, method, properties, body):
            ...
            if len(message_q.qsize() > config.get('AMQP', 'high_water_mark', fallback=100)):
                while message_q.qsize() > config.get('AMQP', 'low_water_mark', fallback=10):
                    logging.error("Sleeping for 1 second to allow the queue to drain")
                    time.sleep(1)
